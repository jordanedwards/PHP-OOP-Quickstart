<?php 
 ...
 include($class_folder . "/class_programs.php");
 ?>
...
<html lang="en">
  <head>
    ...
    <title><?php  echo $appConfig["app_title"];  ?> | Programs List</title>
  ...
  <div class="container">
    ...
    <div class="row">
      ...
      <div class="row">
        ...
        <img src="images/add.png" alt="add" /> <a href="programs_edit.php?id=0">Add New Programs</a></p>
<?php 
...
$dm = new DataManager();
			...
			$s_fee = mysql_real_escape_string($_REQUEST["s_fee"]);
        ...
        <div id="search">
          <form action="<?php  echo $_SERVER["PHP_SELF"]  ?>?reload=true" method="post" name="frmFilter" id="frmFilter">
            <table class="admin_table" style="display:block">
              <tr>
			  <td>Programs Id</td><td>Programs Title</td><td>Programs Fee</td><td>Programs Active</td>				<td><input type="button" class="clear" value="Clear" /></td>
              </tr>
			  
              <tr>
			  <td><input type="text" name="s_id"  value="<?php  echo $_POST["s_id"]  ?>"/></td>
				<td><input type="text" name="s_title"  value="<?php  echo $_POST["s_title"]  ?>"/></td>
				<td><input type="text" name="s_fee"  value="<?php  echo $_POST["s_fee"]  ?>"/></td>
...
<?php 
					...
					// Page set to reload (new query)		
						...
						if($s_id != ""){
								$query_where = ' AND programs_id = "'.$s_id.'"';
						} 
						if($s_title != ""){
								$query_where = ' AND programs_title = "'.$s_title.'"';
						} 
						if($s_fee != ""){
								$query_where = ' AND programs_fee = "'.$s_fee.'"';
						} 
						if($s_active != ""){
								$query_where = ' AND programs_active = "'.$s_active.'"';
						}		

						$query = "SELECT * from programs WHERE 1=1" . $query_where;
							
							
						if ($s_sort != ""){
           					$order = " ORDER BY " . $s_sort . " " . $s_sort_dir;		
						} else {
							$order = " ORDER BY programs_id DESC";
						}
						$query .= $order;
					...
					$pager=new Pager($query,'paginglinks',20,0,1,'page_templates/programs_list_template.htm');
