<?php 
 ...
 include($class_folder . "/class_customerNotes.php");
 ?>
...
<html lang="en">
  <head>
    ...
    <title><?php  echo $appConfig["app_title"];  ?> | CustomerNotes List</title>
  ...
  <div class="container">
    ...
    <div class="row">
      ...
      <div class="row">
        ...
        <img src="images/add.png" alt="add" /> <a href="customerNotes_edit.php?id=0">Add New CustomerNotes</a></p>
<?php 
        ...
        <div id="search">
          <form action="<?php  echo $_SERVER["PHP_SELF"]  ?>?reload=true" method="post" name="frmFilter" id="frmFilter">
            <table class="admin_table" style="display:block">
              <tr>
			  <td>CustomerNotes Id</td><td>CustomerNotes Customer_id</td><td>CustomerNotes Title</td>				<td><input type="button" class="clear" value="Clear" /></td>
              </tr>
...
<?php 
					...
					// Page set to reload (new query)		
						...
						if($s_id != ""){
								$query_where .= ' AND customerNotes_id = "'.$s_id.'"';
						} 
						if($s_customer_id != ""){
								$query_where .= ' AND customerNotes_customer_id = "'.$s_customer_id.'"';
						} 
						if($s_title != ""){
								$query_where .= ' AND customerNotes_title = "'.$s_title.'"';
						}		

						$query = "SELECT * from customerNotes WHERE 1=1" . $query_where .$order;
					...
					$pager=new Pager($query,'paginglinks',20,0,1,'page_templates/customerNotes_list_template.htm');
