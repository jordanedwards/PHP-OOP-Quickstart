<?php 
 ...
 include($class_folder . "/class_settings.php");
 ?>
...
<html lang="en">
  <head>
    ...
    <title><?php  echo $appConfig["app_title"];  ?> | Settings List</title>
...
<?php  include("nav_bar.php") ?>
  ...
  <div class="row">
      ...
      <div class="row">
        ...
        <img src="images/add.png" alt="add" /> <a href="settings_edit.php?id=0">Add New Settings</a></p>
<?php 
...
$dm = new DataManager();
			...
			$s_value = mysqli_real_escape_string($dm->connection, $_REQUEST["s_value"]);
			$s_session_load = mysqli_real_escape_string($dm->connection, $_REQUEST["s_session_load"]);
        ...
        <div id="search">
          <form action="<?php  echo $_SERVER["PHP_SELF"]  ?>?reload=true" method="post" name="frmFilter" id="frmFilter">
            <table class="admin_table" style="display:block">
              <tr>
			  <td>Settings Id</td><td>Settings Name</td><td>Settings Value</td><td>Settings Session_load</td>				<td><input type="button" class="clear" value="Clear" /></td>
              </tr>
			  
 				<tr>
			  <td><input type="text" name="s_id"  value="<?php  echo $_POST["s_id"]  ?>"/></td>
				<td><input type="text" name="s_name"  value="<?php  echo $_POST["s_name"]  ?>"/></td>
				<td><input type="text" name="s_value"  value="<?php  echo $_POST["s_value"]  ?>"/></td>
				<td><input type="text" name="s_session_load"  value="<?php  echo $_POST["s_session_load"]  ?>"/></td>
...
<?php 
					...
					// Page set to reload (new query)		
						...
						if($s_id != ""){
								$query_where .= ' AND settings_id = "'.$s_id.'"';
						} 
						if($s_name != ""){
								$query_where .= ' AND settings_name = "'.$s_name.'"';
						} 
						if($s_value != ""){
								$query_where .= ' AND settings_value = "'.$s_value.'"';
						} 
						if($s_session_load != ""){
								$query_where .= ' AND settings_session_load = "'.$s_session_load.'"';
						}		

						$query = "SELECT * from settings WHERE 1=1" . $query_where .$order;
					...
					$pager=new Pager($query,'paginglinks',20,0,1,'page_templates/settings_list_template.htm');
