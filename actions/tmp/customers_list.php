<?php 
 ...
 include($class_folder . "/class_customers.php");
 ?>
...
<html lang="en">
  <head>
    ...
    <title><?php  echo $appConfig["app_title"];  ?> | Customers List</title>
  ...
  <div class="container">
	<br>	
      <div class="row">
        ...
        <img src="images/add.png" alt="add" /> <a href="customers_edit.php?id=0">Add New Customers</a></p>
<?php 
...
$dm = new DataManager();
			...
			$s_address = mysql_real_escape_string($_REQUEST["s_address"]);
			$s_phone = mysql_real_escape_string($_REQUEST["s_phone"]);
        ...
        <div id="search">
          <form action="<?php  echo $_SERVER["PHP_SELF"]  ?>?reload=true" method="post" name="frmFilter" id="frmFilter">
            <table class="admin_table" style="display:block">
              <tr>
			  <td>Customers Id</td><td>Customers Name</td><td>Customers Address</td><td>Customers Phone</td>				<td><input type="button" class="clear" value="Clear" /></td>
              </tr>
              <tr>
                <td valign="top">
...
<?php 
					...
					// Page set to reload (new query)		
						...
						if($s_id != ""){
								$query_where = ' AND customers_id = "'.$s_id.'"';
						} 
						if($s_name != ""){
								$query_where = ' AND customers_name = "'.$s_name.'"';
						} 
						if($s_address != ""){
								$query_where = ' AND customers_address = "'.$s_address.'"';
						} 
						if($s_phone != ""){
								$query_where = ' AND customers_phone = "'.$s_phone.'"';
						}		

						$query = "SELECT * from customers WHERE 1=1" . $query_where;
							
							
						if ($s_sort != ""){
           					$order = " ORDER BY " . $s_sort . " " . $s_sort_dir;		
						} else {
							$order = " ORDER BY customer_id DESC";
						}
						$query .= $order;
					...
					$pager=new Pager($query,'paginglinks',20,0,1,'page_templates/customers_list_template.htm');
