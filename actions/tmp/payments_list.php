<?php 
 ...
 include($class_folder . "/class_payments.php");
 ?>
...
<html lang="en">
  <head>
    ...
    <title><?php  echo $appConfig["app_title"];  ?> | Payments List</title>
  ...
  <div class="container">
    ...
    <div class="row">
      ...
      <div class="row">
        ...
        <img src="images/add.png" alt="add" /> <a href="payments_edit.php?id=0">Add New Payments</a></p>
<?php 
...
$dm = new DataManager();
			...
			$s_amount = mysql_real_escape_string($_REQUEST["s_amount"]);
			$s_method = mysql_real_escape_string($_REQUEST["s_method"]);
        ...
        <div id="search">
          <form action="<?php  echo $_SERVER["PHP_SELF"]  ?>?reload=true" method="post" name="frmFilter" id="frmFilter">
            <table class="admin_table" style="display:block">
              <tr>
			  <td>Payments Id</td><td>Payments Customer_id</td><td>Payments Amount</td><td>Payments Method</td><td>Payments Status</td>				<td><input type="button" class="clear" value="Clear" /></td>
              </tr>
			  ...
			  <td><input type="text" name="s_id"  value="<?php  echo $_POST["s_id"]  ?>"/></td>
				...
				<td><input type="text" name="s_amount"  value="<?php  echo $_POST["s_amount"]  ?>"/></td>
				<td><input type="text" name="s_method"  value="<?php  echo $_POST["s_method"]  ?>"/></td>
...
<?php 
					...
					// Page set to reload (new query)		
						...
						if($s_id != ""){
								$query_where = ' AND payments_id = "'.$s_id.'"';
						} 
						if($s_customer_id != ""){
								$query_where = ' AND payments_customer_id = "'.$s_customer_id.'"';
						} 
						if($s_amount != ""){
								$query_where = ' AND payments_amount = "'.$s_amount.'"';
						} 
						if($s_method != ""){
								$query_where = ' AND payments_method = "'.$s_method.'"';
						} 
						if($s_status != ""){
								$query_where = ' AND payments_status = "'.$s_status.'"';
						}		

						$query = "SELECT * from payments WHERE 1=1" . $query_where;
							
							
						if ($s_sort != ""){
           					$order = " ORDER BY " . $s_sort . " " . $s_sort_dir;		
						} else {
							$order = " ORDER BY payments_id DESC";
						}
						$query .= $order;
					...
					$pager=new Pager($query,'paginglinks',20,0,1,'page_templates/payments_list_template.htm');
