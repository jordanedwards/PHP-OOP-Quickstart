...
class DropDown {
	...
	public function get_dropdown($name, $id, $table_name, $name_field, $selected_value="None", $just_show_active=false, $required=false, $onchange = '', $order="ASC"){
	if (isset($table_name)):
		try{
			//Find index field:
			$dm = new DataManager();
			...
			if ($num_rows_index > 0){
			while ($row = mysqli_fetch_array($indexResult, MYSQLI_ASSOC)) {
			...
			$requiredClass = ($required ? ' class="{validate:{required:true}}' : "");
			...
			if ($num_rows != 0){
				...
				$ddl = '<select id="'.$id.'" name="'.$name.'" onchange="'.$onchange.'" ' . $requiredClass. '">';
